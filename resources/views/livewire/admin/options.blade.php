                    ...
                    <tbody>
                        @foreach($recentSchedules as $s)
                            <tr>
                                ...
                                <td>
                                    @if($s['type'] === 'truce')
                                        Terre: {{ ($s['payload']['block_earth'] ?? false) ? 'oui' : 'non' }},
                                        Spatial: {{ ($s['payload']['block_spatial'] ?? false) ? 'oui' : 'non' }},
                                        Spy: {{ ($s['payload']['block_spy'] ?? false) ? 'oui' : 'non' }}
                                        @if($s['message'])<br><em>{{ $s['message'] }}</em>@endif
                                    @elseif($s['type'] === 'bonus')
                                        Prod: +{{ (float)($s['payload']['production_bonus_percent'] ?? 0) }}%,
                                        Stock: +{{ (float)($s['payload']['storage_bonus_percent'] ?? 0) }}%
                                        @if($s['message'])<br><em>{{ $s['message'] }}</em>@endif
                                    @elseif($s['type'] === 'shop_bonus')
                                        Boutique: Or +{{ (float)($s['payload']['gold_bonus_percent'] ?? 0) }}%
                                        @if($s['message'])<br><em>{{ $s['message'] }}</em>@endif
                                    @endif
                                </td>
                                <td>
                                ...
                                <td>
                                    <button class="admin-btn admin-btn-sm" wire:click="toggleSchedule({{ $s['id'] }})">
                            ...
                            <tr>
                                ...
                                <td>
                                    <details>
                                        ...
                                        <pre class="admin-code-block">{{ json_encode($log['changes'], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                                    </details>
                                </td>
                            </tr>
                        @endforeach
