...
<div
    class="alpine-editor"
    style="--editor-min-height: {{ $height }};"
    x-data="{ 
        ...
        async uploadImage() {
            ...
            fd.append('image', f);
            const res = await fetch('{{ route('editor.image.store') }}', {
                method: 'POST',
...
>
    @if($label)
        <label class="block mb-2 text-sm font-medium text-slate-300">{{ $label }}</label>
    @endif

    <!-- input hors écran relié à Livewire via x-model: valeur toujours envoyée -->
    <input type="text" x-model="value" {{ $attributes->whereStartsWith('wire:model') }}
           autocomplete="off"
    ...
    <div x-ref="area" wire:ignore class="simple-area" contenteditable="true" data-placeholder="{{ $placeholder }}"></div>
</div>
